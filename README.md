...
This repository contains the files used either in the process of searching pollinators, or the groups of pollinators themselves. To generate a set of all likely pollinating species, the below datasets needs to be merged with the Catalogue of Life (COL) to pull across all the pollinator species names (see that script here https://github.com/Joemillard/Global_effects_of_land-use_intensity_on_local_pollinator-biodiversity/blob/main/R/01c_COL_compilation.R). This script will need some additional checks to be sure it's not duplicating species names. Since the COL subset was not carried out in conjunction with an expert check this set of species should not be taken at face value - omission/commission for the PREDICTS sample provides some idea of confidence for the whole set of COL species.
